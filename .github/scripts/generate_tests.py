...
3. Make sure mocks for async functions return awaitable objects
4. When mocking libraries like aiohttp, the side_effect or return_value MUST be awaitable (like AsyncMock())
5. Always properly define all variables before use
6. For AsyncMock, import it from unittest.mock (from unittest.mock import AsyncMock)
7. NEVER invent module paths that don't exist (e.g., don't treat a module as a package)
8. DO NOT create nested import paths unless they actually exist in the project
9. DO verify that all imports resolve correctly for the project structure
